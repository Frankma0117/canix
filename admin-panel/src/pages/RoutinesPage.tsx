...
function RoutineRow({ routine, categoryName, onDeleted }: { routine: Todo; categoryName: string; onDeleted: () => void }) {
  const api = useApi();
    ...
    <div className="mx-auto max-w-3xl p-8">
      ...
      {routines && routines.length > 0 && (
        <div className="space-y-2">
          {routines.map((r) => (
            <RoutineRow key={r.id} routine={r} categoryName={categoryName(r.category_id)} onDeleted={load} />
          ))}
        </div>
      )}
